...
- (instancetype)init {
    self = [super init];
    if (self) {
        UIColor *foregroundColor = UIApplication.sharedApplication.keyWindow.tintColor ? : UIColor.redColor;
        self.descriptionLinkAttributes = @{NSForegroundColorAttributeName : foregroundColor};
...
- (NSAttributedString*)truncationString {
    ...
    NSDictionary *truncationAttributes = @{NSFontAttributeName : [UIFont systemFontOfSize:14],
                                           NSForegroundColorAttributeName : UIColor.whiteColor};
    NSMutableAttributedString *truncation = [NSMutableAttributedString.alloc initWithString:wholeString
                                                                                 attributes:truncationAttributes];
    
    UIColor *keyColor = UIApplication.sharedApplication.keyWindow.tintColor ? : UIColor.whiteColor;
...
- (void)setMHGalleryBackgroundColor:(UIColor *)color
    ...
    [self.backgroundColorsForViewModes setObject:color
                                          forKey:[NSString stringWithFormat:@"%@",@(viewMode)]];
}
